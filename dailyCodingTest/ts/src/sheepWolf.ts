namespace SheepWolf {
  class Tree {
    info;
    edges;
    hashedEdges: { [idx: number]: number[] };
    constructor(info: number[], edges: number[][]) {
      this.info = info;
      this.edges = edges;
      this.hashedEdges = {};
  ...
  class Work {
    ...
    wolf: number;
    next: Work | null;
    constructor(size: number, visited: number[]) {
  ...
  export function solution(info: number[], edges: number[][]) {
    ...
    while (queue.length > 0) {
      const work = queue.dequeue() as Work;
