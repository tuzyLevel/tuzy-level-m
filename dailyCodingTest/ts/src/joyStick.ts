const closeMoveCountFromA = (char: string) => {
...
const getDirectionNEndPoint = (name: string) => {
  let rightEmptyLength = 0;
  let leftEmptyLength = 0;
  for (let i = 1; i < name.length / 2; i++) {
    if (name[name.length - i] === "A") leftEmptyLength++;
    if (name[i] === "A") rightEmptyLength++;
