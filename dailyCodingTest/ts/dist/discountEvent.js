"use strict";
const checkDiscountList = (want, number, window) => {
    for (let i = 0; i < want.length; i++) {
        if (number[i] !== window[want[i]])
            return false;
    }
    return true;
};
const discountEvent = (want, number, discount) => {
    let result = 0;
...
const _ = (want, number, discount) => {
    return discountEvent(want, number, discount);
};
console.log(_(["banana", "apple", "rice", "pork", "pot"], [3, 2, 2, 2, 1], [
    "chicken",
    "apple",
    "apple",
    "banana",
